...
module arm_joint(h) {
    translate([- width / 2 - 0.01, 3, -height + 10]) {
        cylinder_with_hole(h, width * 1.5, width * 1.5, h, width / 2, width / 2);
    }
}
        ...
        union() {
            difference() {
                ...
                if (measure_holes) {
                    pos = 100;
                    ir = width / 4;
                    translate([17.5, pos + 1, -(height * 0.05)]) {
                        cylinder(height * 1.1, ir, ir);
                    }
                    translate([21, pos + 10, - (height * 0.05)]) {
                        cylinder(height * 1.1, ir, ir);
                    }
                }
            }
...
mirror([0, 0, 1]) {
    ...
    rotate([0, 0, 10]) translate ([-20, 0, - height]) arm(1);
